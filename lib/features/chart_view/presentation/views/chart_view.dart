...
var startDate;
  ...
  void _show() async {
    ...
    if (result != null) {
      ...
      setState(() {
        ...
        startDate =result.start.millisecondsSinceEpoch.toString().substring(0,10);
        endDate =result.end.millisecondsSinceEpoch.toString().substring(0,10);
        print(startDate);
        print(endDate);
        Get.find<AppController>().openFile(
            url:
            "https://query1.finance.yahoo.com/v7/finance/download/SPUS?period1=$startDate&period2=$endDate&interval=1d&events=history&crumb=5YTX%2FgVGBmg",
            fileName: "finance.csv");
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      bottomNavigationBar: Container(width: Get.width,
      height: 80.h,
        child: Row(children: [
          SizedBox(width: 10.w,),
